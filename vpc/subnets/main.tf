...
resource "aws_subnet" "mod_private" {
  vpc_id = "${var.vpc_id}"
  cidr_block = "${cidrsubnet(var.vpc_cidr_block, var.private_newbits, var.private_netnum_offset + lookup(var.az_to_netnum, substr(element(data.aws_availability_zones.available.names, count.index), -1, 1)))}"
...
resource "aws_subnet" "mod_public" {
  vpc_id = "${var.vpc_id}"
  cidr_block = "${cidrsubnet(var.vpc_cidr_block, var.public_newbits, var.public_netnum_offset + lookup(var.az_to_netnum, substr(element(data.aws_availability_zones.available.names, count.index), -1, 1)))}"
  availability_zone = "${element(data.aws_availability_zones.available.names, count.index)}"
  count = "${length(data.aws_availability_zones.available.names)}"
